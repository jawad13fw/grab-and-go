...
const Home = () => {
  ...
  const featuredShops = useMemo(() => {
    ...
    // Prioritize the local Vehari anchors users expect to see first.
    const priorityIds = [
      'vehari-grocery-1',
      'vehari-grocery-2',
      'vehari-grocery-3',
    ...
    const fallback = shops.filter((shop) => !priorityIds.includes(shop.id));
  ...
  const categoryRecommendations = useMemo(() => {
    ...
    return categories
      .filter(cat => ['grocery', 'electronics', 'computers', 'fashion', 'cosmetics', 'pharmacy'].includes(cat.id))
      .map(cat => ({
        ...cat,
  ...
  const marketplaceSnapshot = useMemo(() => {
    ...
    const groceryProducts = products.filter((p) => p.shopId?.startsWith('vehari-grocery-')).length;
    const electronicsProducts = products.filter((p) => p.shopId?.startsWith('vehari-mobile-')).length;
    const localImageProducts = products.filter(
      (p) => typeof p.image === 'string' && p.image.startsWith('/product-images/')
    ).length;
  ...
  return (
    <div className="space-y-8 sm:space-y-12 lg:space-y-16">
      ...
      <section className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h2 className="text-xl sm:text-2xl font-semibold text-slate-900">Marketplace Snapshot</h2>
            <p className="text-sm text-slate-500 mt-1">Live view of what is currently loaded on home</p>
          </div>
        </div>
        <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-5">
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Total Shops</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">{marketplaceSnapshot.totalShops}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Total Products</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">{marketplaceSnapshot.totalProducts}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Grocery Products</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">{marketplaceSnapshot.groceryProducts}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <p className="text-xs uppercase tracking-wide text-slate-500">Electronics Products</p>
            <p className="mt-2 text-2xl font-bold text-slate-900">{marketplaceSnapshot.electronicsProducts}</p>
          </div>
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <p className="text-xs uppercase tracking-wide text-emerald-700">Local Images</p>
            <p className="mt-2 text-2xl font-bold text-emerald-800">{marketplaceSnapshot.localImageProducts}</p>
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div className="p-4">
                    <h4 className="font-semibold text-slate-900">
                      {category.id === 'computers' ? 'Mobile & Computer Shops' : category.name}
                    </h4>
