...
const SearchResults = () => {
  ...
  // Suggestions for the search bar: products first, then shops, then categories last
  const searchSuggestions = useMemo(() => {
    const productSuggestions = products.slice(0, 6).map((item) => ({
      label: item.name,
      value: item.name,
      meta: `Product${item.shopName ? ` · ${item.shopName}` : ''}${item.category ? ` (${item.category})` : ''}`,
    }));

    const shopSuggestions = shops.slice(0, 4).map((item) => ({
      label: item.name,
      value: item.name,
      meta: `Shop${item.category ? ` · ${item.category}` : ''}`,
    }));

    // Categories only at the very end, and only a few
    const categorySuggestions = (categories || []).slice(0, 3).map((item) => ({
      label: item.name || item.id,
      value: item.name || item.id,
      meta: 'Category',
    }));

    // Products first, shops second, categories last
    return [...productSuggestions, ...shopSuggestions, ...categorySuggestions];
  }, [categories, products, shops]);

  // Dynamic suggestion fetcher for live search-as-you-type
  const fetchSuggestions = useCallback(async (searchQuery) => {
    if (!searchQuery || searchQuery.length < 2) return [];
    try {
      const [productsRes, shopsRes] = await Promise.allSettled([
        catalogApi.searchProducts({ q: searchQuery, limit: 5 }),
        catalogApi.searchShops({ q: searchQuery, limit: 3 }),
      ]);

      const prodSugs = (productsRes.status === 'fulfilled'
      ...
      const shopSugs = (shopsRes.status === 'fulfilled'
        ? (shopsRes.value?.shops || shopsRes.value || [])
        : []
      ).slice(0, 3).map((s) => ({
        label: s.name,
        value: s.name,
        meta: `Shop${s.category ? ` · ${s.category}` : ''}`,
      }));

      return [...prodSugs, ...shopSugs];
    } catch {
      return [];
    }
  }, []);
    ...
    <div className="space-y-8">
      ...
      <div className="max-w-2xl mx-auto">
        <SearchBar
          ...
          recentSearchKey="search-results-searches"
          helperText="Type product or shop names to search"
        />
      </div>
